...
class AuthMiddleware {
  ...
  //     if (!!!token)
  //       return res.status(400).send({ error: "x-access-token not found" });
  //     if (token instanceof Array)
  //       return res.status(400).send({ error: "x-access-token invalid format" });
  //     try {
  //       const result: any = AuthJwt.isAuthenticated(token);
  //       if (await AuthJwt.isBlackList(token))
  //         throw new Error("x-access-token in black list");
